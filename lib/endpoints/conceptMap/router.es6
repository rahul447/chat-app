...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/conceptMap`,
  ...
  conceptMapRootRoute = router.route("/"),
  conceptMapParamRoute = router.route("/:id");

conceptMapParamRoute
  .get(serviceMapperInstance.retrieveConceptMap.bind(serviceMapperInstance));

conceptMapRootRoute
  .post(serviceMapperInstance.createConceptMap.bind(serviceMapperInstance));
