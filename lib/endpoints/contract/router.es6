...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/contract`,
  ...
  contractRootRoute = router.route("/"),
  contractParamRoute = router.route("/:id");

contractParamRoute
  .get(serviceMapperInstance.retrieveContract.bind(serviceMapperInstance));

contractRootRoute
  .post(serviceMapperInstance.createContract.bind(serviceMapperInstance));
