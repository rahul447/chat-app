...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/schedule`,
  ...
  scheduleRootRoute = router.route("/"),
  scheduleParamRoute = router.route("/:id");

scheduleParamRoute
  .get(serviceMapperInstance.retrieveSchedule.bind(serviceMapperInstance));

scheduleRootRoute
  .post(serviceMapperInstance.createSchedule.bind(serviceMapperInstance));
