...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/messageHeader`,
  ...
  messageHeaderRootRoute = router.route("/"),
  messageHeaderParamRoute = router.route("/:id");

messageHeaderParamRoute
  .get(serviceMapperInstance.retrieveMessageHeader.bind(serviceMapperInstance));

messageHeaderRootRoute
  .post(serviceMapperInstance.createMessageHeader.bind(serviceMapperInstance));
