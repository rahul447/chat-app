...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/slot`,
  ...
  slotRootRoute = router.route("/"),
  slotParamRoute = router.route("/:id");

slotParamRoute
  .get(serviceMapperInstance.retrieveSlot.bind(serviceMapperInstance));

slotRootRoute
  .post(serviceMapperInstance.createSlot.bind(serviceMapperInstance));
