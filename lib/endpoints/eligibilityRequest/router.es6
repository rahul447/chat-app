...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/eligibilityRequest`,
  ...
  eligibilityRequestRootRoute = router.route("/"),
  eligibilityRequestParamRoute = router.route("/:id");

eligibilityRequestParamRoute
  .get(serviceMapperInstance.retrieveEligibilityRequest.bind(serviceMapperInstance));

eligibilityRequestRootRoute
  .post(serviceMapperInstance.createEligibilityRequest.bind(serviceMapperInstance));
