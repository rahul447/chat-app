...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/procedureRequest`,
  ...
  procedureRequestRootRoute = router.route("/"),
  procedureRequestParamRoute = router.route("/:id");

procedureRequestParamRoute
  .get(serviceMapperInstance.retrieveProcedureRequest.bind(serviceMapperInstance));

procedureRequestRootRoute
  .post(serviceMapperInstance.createProcedureRequest.bind(serviceMapperInstance));
