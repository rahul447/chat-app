...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/nutritionOrder`,
  ...
  nutritionOrderRootRoute = router.route("/"),
  nutritionOrderParamRoute = router.route("/:id");

nutritionOrderParamRoute
  .get(serviceMapperInstance.retrieveNutritionOrder.bind(serviceMapperInstance));

nutritionOrderRootRoute
  .post(serviceMapperInstance.createNutritionOrder.bind(serviceMapperInstance));
