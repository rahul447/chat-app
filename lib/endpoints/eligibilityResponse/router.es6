...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/eligibilityResponse`,
  ...
  eligibilityResponseRootRoute = router.route("/"),
  eligibilityResponseParamRoute = router.route("/:id");

eligibilityResponseParamRoute
  .get(serviceMapperInstance.retrieveEligibilityResponse.bind(serviceMapperInstance));

eligibilityResponseRootRoute
  .post(serviceMapperInstance.createEligibilityResponse.bind(serviceMapperInstance));
