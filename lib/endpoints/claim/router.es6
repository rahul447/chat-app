...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/claim`,
  ...
  claimRootRoute = router.route("/"),
  claimParamRoute = router.route("/:id");

claimParamRoute
  .get(serviceMapperInstance.retrieveClaim.bind(serviceMapperInstance));

claimRootRoute
  .post(serviceMapperInstance.createClaim.bind(serviceMapperInstance));
