...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/diagnosticOrder`,
  ...
  diagnosticOrderRootRoute = router.route("/"),
  diagnosticOrderParamRoute = router.route("/:id");

diagnosticOrderParamRoute
  .get(serviceMapperInstance.retrieveDiagnosticOrder.bind(serviceMapperInstance));

diagnosticOrderRootRoute
  .post(serviceMapperInstance.createDiagnosticOrder.bind(serviceMapperInstance));
