...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/bodySite`,
  ...
  bodySiteRootRoute = router.route("/"),
  bodySiteParamRoute = router.route("/:id");

bodySiteParamRoute
  .get(serviceMapperInstance.retrieveBodySite.bind(serviceMapperInstance));

bodySiteRootRoute
  .post(serviceMapperInstance.createBodySite.bind(serviceMapperInstance));
