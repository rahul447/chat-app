...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/communication`,
  ...
  communicationRootRoute = router.route("/"),
  communicationParamRoute = router.route("/:id");

communicationParamRoute
  .get(serviceMapperInstance.retrieveCommunication.bind(serviceMapperInstance));

communicationRootRoute
  .post(serviceMapperInstance.createCommunication.bind(serviceMapperInstance));
