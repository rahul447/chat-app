...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/enrollmentResponse`,
  ...
  enrollmentResponseRootRoute = router.route("/"),
  enrollmentResponseParamRoute = router.route("/:id");

enrollmentResponseParamRoute
  .get(serviceMapperInstance.retrieveEnrollmentResponse.bind(serviceMapperInstance));

enrollmentResponseRootRoute
  .post(serviceMapperInstance.createEnrollmentResponse.bind(serviceMapperInstance));
