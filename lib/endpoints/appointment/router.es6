...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/appointment`,
  ...
  appointmentRootRoute = router.route("/"),
  appointmentParamRoute = router.route("/:id");

appointmentParamRoute
  .get(serviceMapperInstance.retrieveAppointment.bind(serviceMapperInstance));

appointmentRootRoute
  .post(serviceMapperInstance.createAppointment.bind(serviceMapperInstance));
