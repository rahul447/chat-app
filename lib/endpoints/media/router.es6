...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/media`,
  ...
  mediaRootRoute = router.route("/"),
  mediaParamRoute = router.route("/:id");

mediaParamRoute
  .get(serviceMapperInstance.retrieveMedia.bind(serviceMapperInstance));

mediaRootRoute
  .post(serviceMapperInstance.createMedia.bind(serviceMapperInstance));
