...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/namingSystem`,
  ...
  namingSystemRootRoute = router.route("/"),
  namingSystemParamRoute = router.route("/:id");

namingSystemParamRoute
  .get(serviceMapperInstance.retrieveNamingSystem.bind(serviceMapperInstance));

namingSystemRootRoute
  .post(serviceMapperInstance.createNamingSystem.bind(serviceMapperInstance));
