...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/auditEvent`,
  ...
  auditEventRootRoute = router.route("/"),
  auditEventParamRoute = router.route("/:id");

auditEventParamRoute
  .get(serviceMapperInstance.retrieveAuditEvent.bind(serviceMapperInstance));

auditEventRootRoute
  .post(serviceMapperInstance.createAuditEvent.bind(serviceMapperInstance));
