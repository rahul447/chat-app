...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/bundle`,
  ...
  bundleRootRoute = router.route("/"),
  bundleParamRoute = router.route("/:id");

bundleParamRoute
  .get(serviceMapperInstance.retrieveBundle.bind(serviceMapperInstance));

bundleRootRoute
  .post(serviceMapperInstance.createBundle.bind(serviceMapperInstance));
