...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/processResponse`,
  ...
  processResponseRootRoute = router.route("/"),
  processResponseParamRoute = router.route("/:id");

processResponseParamRoute
  .get(serviceMapperInstance.retrieveProcessResponse.bind(serviceMapperInstance));

processResponseRootRoute
  .post(serviceMapperInstance.createProcessResponse.bind(serviceMapperInstance));
