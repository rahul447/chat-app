...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/list`,
  ...
  listRootRoute = router.route("/"),
  listParamRoute = router.route("/:id");

listParamRoute
  .get(serviceMapperInstance.retrieveList.bind(serviceMapperInstance));

listRootRoute
  .post(serviceMapperInstance.createList.bind(serviceMapperInstance));
