...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/parameters`,
  ...
  parametersRootRoute = router.route("/"),
  parametersParamRoute = router.route("/:id");

parametersParamRoute
  .get(serviceMapperInstance.retrieveParameters.bind(serviceMapperInstance));

parametersRootRoute
  .post(serviceMapperInstance.createParameters.bind(serviceMapperInstance));
