...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/allergyIntolerance`,
  ...
  allergyIntoleranceRootRoute = router.route("/"),
  allergyIntoleranceParamRoute = router.route("/:id");

allergyIntoleranceParamRoute
  .get(serviceMapperInstance.retrieveAllergyIntolerance.bind(serviceMapperInstance));

allergyIntoleranceRootRoute
  .post(serviceMapperInstance.createAllergyIntolerance.bind(serviceMapperInstance));
