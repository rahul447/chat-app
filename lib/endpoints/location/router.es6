...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/location`,
  ...
  locationRootRoute = router.route("/"),
  locationParamRoute = router.route("/:id");

locationParamRoute
  .get(serviceMapperInstance.retrieveLocation.bind(serviceMapperInstance));

locationRootRoute
  .post(serviceMapperInstance.createLocation.bind(serviceMapperInstance));
