...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/dataElement`,
  ...
  dataElementRootRoute = router.route("/"),
  dataElementParamRoute = router.route("/:id");

dataElementParamRoute
  .get(serviceMapperInstance.retrieveDataElement.bind(serviceMapperInstance));

dataElementRootRoute
  .post(serviceMapperInstance.createDataElement.bind(serviceMapperInstance));
