...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/testScript`,
  ...
  testScriptRootRoute = router.route("/"),
  testScriptParamRoute = router.route("/:id");

testScriptParamRoute
  .get(serviceMapperInstance.retrieveTestScript.bind(serviceMapperInstance));

testScriptRootRoute
  .post(serviceMapperInstance.createTestScript.bind(serviceMapperInstance));
