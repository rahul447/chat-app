...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/coverage`,
  ...
  coverageRootRoute = router.route("/"),
  coverageParamRoute = router.route("/:id");

coverageParamRoute
  .get(serviceMapperInstance.retrieveCoverage.bind(serviceMapperInstance));

coverageRootRoute
  .post(serviceMapperInstance.createCoverage.bind(serviceMapperInstance));
