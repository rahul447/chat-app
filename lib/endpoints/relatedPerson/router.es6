...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/relatedPerson`,
  ...
  relatedPersonRootRoute = router.route("/"),
  relatedPersonParamRoute = router.route("/:id");

relatedPersonParamRoute
  .get(serviceMapperInstance.retrieveRelatedPerson.bind(serviceMapperInstance));

relatedPersonRootRoute
  .post(serviceMapperInstance.createRelatedPerson.bind(serviceMapperInstance));
