...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/substance`,
  ...
  substanceRootRoute = router.route("/"),
  substanceParamRoute = router.route("/:id");

substanceParamRoute
  .get(serviceMapperInstance.retrieveSubstance.bind(serviceMapperInstance));

substanceRootRoute
  .post(serviceMapperInstance.createSubstance.bind(serviceMapperInstance));
