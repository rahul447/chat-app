...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/conformance`,
  ...
  conformanceRootRoute = router.route("/"),
  conformanceParamRoute = router.route("/:id");

conformanceParamRoute
  .get(serviceMapperInstance.retrieveConformance.bind(serviceMapperInstance));

conformanceRootRoute
  .post(serviceMapperInstance.createConformance.bind(serviceMapperInstance));
