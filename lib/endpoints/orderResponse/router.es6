...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/orderResponse`,
  ...
  orderResponseRootRoute = router.route("/"),
  orderResponseParamRoute = router.route("/:id");

orderResponseParamRoute
  .get(serviceMapperInstance.retrieveOrderResponse.bind(serviceMapperInstance));

orderResponseRootRoute
  .post(serviceMapperInstance.createOrderResponse.bind(serviceMapperInstance));
