...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/deviceUseRequest`,
  ...
  deviceUseRequestRootRoute = router.route("/"),
  deviceUseRequestParamRoute = router.route("/:id");

deviceUseRequestParamRoute
  .get(serviceMapperInstance.retrieveDeviceUseRequest.bind(serviceMapperInstance));

deviceUseRequestRootRoute
  .post(serviceMapperInstance.createDeviceUseRequest.bind(serviceMapperInstance));
