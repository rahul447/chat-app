...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/goal`,
  ...
  goalRootRoute = router.route("/"),
  goalParamRoute = router.route("/:id");

goalParamRoute
  .get(serviceMapperInstance.retrieveGoal.bind(serviceMapperInstance));

goalRootRoute
  .post(serviceMapperInstance.createGoal.bind(serviceMapperInstance));
