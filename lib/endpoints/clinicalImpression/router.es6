...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/clinicalImpression`,
  ...
  clinicalImpressionRootRoute = router.route("/"),
  clinicalImpressionParamRoute = router.route("/:id");

clinicalImpressionParamRoute
  .get(serviceMapperInstance.retrieveClinicalImpression.bind(serviceMapperInstance));

clinicalImpressionRootRoute
  .post(serviceMapperInstance.createClinicalImpression.bind(serviceMapperInstance));
