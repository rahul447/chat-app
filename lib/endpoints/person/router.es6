...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/person`,
  ...
  personRootRoute = router.route("/"),
  personParamRoute = router.route("/:id");

personParamRoute
  .get(serviceMapperInstance.retrievePerson.bind(serviceMapperInstance));

personRootRoute
  .post(serviceMapperInstance.createPerson.bind(serviceMapperInstance));
