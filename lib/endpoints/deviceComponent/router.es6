...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/deviceComponent`,
  ...
  deviceComponentRootRoute = router.route("/"),
  deviceComponentParamRoute = router.route("/:id");

deviceComponentParamRoute
  .get(serviceMapperInstance.retrieveDeviceComponent.bind(serviceMapperInstance));

deviceComponentRootRoute
  .post(serviceMapperInstance.createDeviceComponent.bind(serviceMapperInstance));
