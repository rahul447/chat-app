...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/healthcareService`,
  ...
  healthcareServiceRootRoute = router.route("/"),
  healthcareServiceParamRoute = router.route("/:id");

healthcareServiceParamRoute
  .get(serviceMapperInstance.retrieveHealthcareService.bind(serviceMapperInstance));

healthcareServiceRootRoute
  .post(serviceMapperInstance.createHealthcareService.bind(serviceMapperInstance));
