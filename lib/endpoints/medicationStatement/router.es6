...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/medicationStatement`,
  ...
  medicationStatementRootRoute = router.route("/"),
  medicationStatementParamRoute = router.route("/:id");

medicationStatementParamRoute
  .get(serviceMapperInstance.retrieveMedicationStatement.bind(serviceMapperInstance));

medicationStatementRootRoute
  .post(serviceMapperInstance.createMedicationStatement.bind(serviceMapperInstance));
