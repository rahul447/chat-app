...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/medicationOrder`,
  ...
  medicationOrderRootRoute = router.route("/"),
  medicationOrderParamRoute = router.route("/:id");

medicationOrderParamRoute
  .get(serviceMapperInstance.retrieveMedicationOrder.bind(serviceMapperInstance));

medicationOrderRootRoute
  .post(serviceMapperInstance.createMedicationOrder.bind(serviceMapperInstance));
