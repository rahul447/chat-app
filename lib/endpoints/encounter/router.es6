...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/encounter`,
  ...
  encounterRootRoute = router.route("/"),
  encounterParamRoute = router.route("/:id");

encounterParamRoute
  .get(serviceMapperInstance.retrieveEncounter.bind(serviceMapperInstance));

encounterRootRoute
  .post(serviceMapperInstance.createEncounter.bind(serviceMapperInstance));
