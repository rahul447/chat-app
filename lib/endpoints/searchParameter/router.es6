...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/searchParameter`,
  ...
  searchParameterRootRoute = router.route("/"),
  searchParameterParamRoute = router.route("/:id");

searchParameterParamRoute
  .get(serviceMapperInstance.retrieveSearchParameter.bind(serviceMapperInstance));

searchParameterRootRoute
  .post(serviceMapperInstance.createSearchParameter.bind(serviceMapperInstance));
