...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/paymentNotice`,
  ...
  paymentNoticeRootRoute = router.route("/"),
  paymentNoticeParamRoute = router.route("/:id");

paymentNoticeParamRoute
  .get(serviceMapperInstance.retrievePaymentNotice.bind(serviceMapperInstance));

paymentNoticeRootRoute
  .post(serviceMapperInstance.createPaymentNotice.bind(serviceMapperInstance));
