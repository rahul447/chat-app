...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/enrollmentRequest`,
  ...
  enrollmentRequestRootRoute = router.route("/"),
  enrollmentRequestParamRoute = router.route("/:id");

enrollmentRequestParamRoute
  .get(serviceMapperInstance.retrieveEnrollmentRequest.bind(serviceMapperInstance));

enrollmentRequestRootRoute
  .post(serviceMapperInstance.createEnrollmentRequest.bind(serviceMapperInstance));
