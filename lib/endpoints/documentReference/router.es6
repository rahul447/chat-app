...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/documentReference`,
  ...
  documentReferenceRootRoute = router.route("/"),
  documentReferenceParamRoute = router.route("/:id");

documentReferenceParamRoute
  .get(serviceMapperInstance.retrieveDocumentReference.bind(serviceMapperInstance));

documentReferenceRootRoute
  .post(serviceMapperInstance.createDocumentReference.bind(serviceMapperInstance));
