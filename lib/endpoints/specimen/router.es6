...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/specimen`,
  ...
  specimenRootRoute = router.route("/"),
  specimenParamRoute = router.route("/:id");

specimenParamRoute
  .get(serviceMapperInstance.retrieveSpecimen.bind(serviceMapperInstance));

specimenRootRoute
  .post(serviceMapperInstance.createSpecimen.bind(serviceMapperInstance));
