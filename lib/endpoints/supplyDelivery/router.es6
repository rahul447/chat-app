...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/supplyDelivery`,
  ...
  supplyDeliveryRootRoute = router.route("/"),
  supplyDeliveryParamRoute = router.route("/:id");

supplyDeliveryParamRoute
  .get(serviceMapperInstance.retrieveSupplyDelivery.bind(serviceMapperInstance));

supplyDeliveryRootRoute
  .post(serviceMapperInstance.createSupplyDelivery.bind(serviceMapperInstance));
