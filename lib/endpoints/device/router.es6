...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/device`,
  ...
  deviceRootRoute = router.route("/"),
  deviceParamRoute = router.route("/:id");

deviceParamRoute
  .get(serviceMapperInstance.retrieveDevice.bind(serviceMapperInstance));

deviceRootRoute
  .post(serviceMapperInstance.createDevice.bind(serviceMapperInstance));
