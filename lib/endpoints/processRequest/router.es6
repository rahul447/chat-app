...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/processRequest`,
  ...
  processRequestRootRoute = router.route("/"),
  processRequestParamRoute = router.route("/:id");

processRequestParamRoute
  .get(serviceMapperInstance.retrieveProcessRequest.bind(serviceMapperInstance));

processRequestRootRoute
  .post(serviceMapperInstance.createProcessRequest.bind(serviceMapperInstance));
