...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/medication`,
  ...
  medicationRootRoute = router.route("/"),
  medicationParamRoute = router.route("/:id");

medicationParamRoute
  .get(serviceMapperInstance.retrieveMedication.bind(serviceMapperInstance));

medicationRootRoute
  .post(serviceMapperInstance.createMedication.bind(serviceMapperInstance));
