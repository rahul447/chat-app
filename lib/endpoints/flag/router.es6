...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/flag`,
  ...
  flagRootRoute = router.route("/"),
  flagParamRoute = router.route("/:id");

flagParamRoute
  .get(serviceMapperInstance.retrieveFlag.bind(serviceMapperInstance));

flagRootRoute
  .post(serviceMapperInstance.createFlag.bind(serviceMapperInstance));
