...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/claimResponse`,
  ...
  claimResponseRootRoute = router.route("/"),
  claimResponseParamRoute = router.route("/:id");

claimResponseParamRoute
  .get(serviceMapperInstance.retrieveClaimResponse.bind(serviceMapperInstance));

claimResponseRootRoute
  .post(serviceMapperInstance.createClaimResponse.bind(serviceMapperInstance));
