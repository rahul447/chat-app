...
export class ServiceMapper {
  ...
  createObservation(req, res, next) {
    ...
    this.mapperMaster.processCudRequest({
      ...
      "event": this.events.fhirResources.observation.create,
  ...
  retrieveObservation(req, res, next) {
    console.log("========= Retrieve Observation ===========");
    console.log(req.params);
    let obj = {
      "id": req.params.id,
      "service": this.serviceInstance,
      "event": this.events.fhirResources.observation.retrieve,
      "res": res,
      "req": req,
      "next": next
    };
