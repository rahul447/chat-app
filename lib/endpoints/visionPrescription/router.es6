...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/visionPrescription`,
  ...
  visionPrescriptionRootRoute = router.route("/"),
  visionPrescriptionParamRoute = router.route("/:id");

visionPrescriptionParamRoute
  .get(serviceMapperInstance.retrieveVisionPrescription.bind(serviceMapperInstance));

visionPrescriptionRootRoute
  .post(serviceMapperInstance.createVisionPrescription.bind(serviceMapperInstance));
