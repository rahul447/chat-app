...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/documentManifest`,
  ...
  documentManifestRootRoute = router.route("/"),
  documentManifestParamRoute = router.route("/:id");

documentManifestParamRoute
  .get(serviceMapperInstance.retrieveDocumentManifest.bind(serviceMapperInstance));

documentManifestRootRoute
  .post(serviceMapperInstance.createDocumentManifest.bind(serviceMapperInstance));
