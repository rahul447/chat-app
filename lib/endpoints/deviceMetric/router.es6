...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/deviceMetric`,
  ...
  deviceMetricRootRoute = router.route("/"),
  deviceMetricParamRoute = router.route("/:id");

deviceMetricParamRoute
  .get(serviceMapperInstance.retrieveDeviceMetric.bind(serviceMapperInstance));

deviceMetricRootRoute
  .post(serviceMapperInstance.createDeviceMetric.bind(serviceMapperInstance));
