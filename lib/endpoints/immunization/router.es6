...
let {NODE_ENV} = process.env,
  ...
  urlBase = `${config.http.protocol}://${config.http.domain}:${config.http.port}/immunization`,
  ...
  immunizationRootRoute = router.route("/"),
  immunizationParamRoute = router.route("/:id");

immunizationParamRoute
  .get(serviceMapperInstance.retrieveImmunization.bind(serviceMapperInstance));

immunizationRootRoute
  .post(serviceMapperInstance.createImmunization.bind(serviceMapperInstance));
